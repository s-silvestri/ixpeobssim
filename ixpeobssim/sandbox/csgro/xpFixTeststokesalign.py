...
from ixpeobssim.utils.math_ import phi_radial
...
PARSER.add_boolean('--rotate', default=False,
                   help='rotate the input maps in XY or PDA mode')
...
def xpstokesalign(**kwargs):
    ...
    for file_path in file_list:
        ...
        if os.path.exists(outfile) and not kwargs.get('overwrite'):
            ...
            continue
        # Open the input file and recover the relevanr columns.
        ...
        # HARDCODED ROTATION ANGLE FOR THIS TEST
        # ASSUMING DU1&2 WERE SWITCHED IN ORIGINAL PROCESSING
        # DU1 need phi_obs = phi_det -120 = phi_det - (+120)
        #phi0 = numpy.deg2rad(+120.)
        # DU2 need phi_obs = phi_det +120 = phi_det - (-120)
        #phi0 = numpy.deg2rad(-120.)
        # DU3 need NO Correction
        #phi0 = 0.
        
        # ASSUMING THERE WAS A SIGN CHANGE
        # DU1 need phi_obs = phi_det +218 = phi_det - (-218)
        phi0 = numpy.deg2rad(-218.)
        # DU2 need phi_obs = phi_det +98 = phi_det - (-98)
        phi0 = numpy.deg2rad(-98.)
        # DU3 need phi_obs = phi_det -22 = phi_det - (+22)
        phi0 = numpy.deg2rad(+22)
        
        # Recalculate the Stokes parameters.
        # Beware we should not fool ourselves and confuse the model Stokes
        # parameters and the event-by-event Stokes parameters (there's a factor
        # of 2 difference). When we get rid of PHI altogether we might want to
        # revisit all of this.
